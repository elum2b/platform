...
var (
	listKey         = "calendar.callback.list"
	listDescription = `
Lists calendar callback delivery events and their states. Requires the
'calendar.callback.list' permission in the target workspace.`
)

// List exposes the calendar callback event listing method.
var List = adapter.Method[ListRequest, ListResponse]{
	...
	Handler: func(ctx *adapter.Context, data ListRequest) (ListResponse, error) {
		values, err := services.Calendar.Admin.ListCallbackEvents(
			ctx.Context,
			caladmin.CallbackEventListParams{
