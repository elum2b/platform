...
var (
	markOKKey         = "calendar.callback.mark_ok"
	markOKDescription = `
Marks a calendar callback event as successfully delivered. Requires the
'calendar.callback.mark_ok' permission in the target workspace.`
)

// MarkOK exposes the calendar callback success method.
var MarkOK = adapter.Method[MarkOKRequest, MarkOKResponse]{
	...
	Handler: func(ctx *adapter.Context, data MarkOKRequest) (MarkOKResponse, error) {
		affected, err := services.Calendar.Admin.MarkCallbackEventOK(
			ctx.Context,
			data.WorkspaceID,
			data.ID,
		)
