...
var (
	getKey         = "calendar.callback.get"
	getDescription = `
Returns a calendar callback delivery event. Requires the
'calendar.callback.get' permission in the target workspace.`
)

// Get exposes the calendar callback event retrieval method.
var Get = adapter.Method[GetRequest, GetResponse]{
	...
	Handler: func(ctx *adapter.Context, data GetRequest) (GetResponse, error) {
		v, err := services.Calendar.Admin.GetCallbackEvent(
			ctx.Context,
			data.WorkspaceID,
			data.ID,
		)
