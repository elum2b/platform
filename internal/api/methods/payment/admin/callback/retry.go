...
var (
	retryKey         = "payment.callback.retry"
	retryDescription = `
Schedules a payment callback event for immediate retry. Requires the
'payment.callback.retry' permission in the target workspace.`
)
...
var Retry = adapter.Method[RetryRequest, RetryResponse]{
	...
	Handler: func(ctx *adapter.Context, data RetryRequest) (RetryResponse, error) {
		affected, err := services.Payment.Admin.RetryCallbackEventNow(
			ctx.Context,
			data.WorkspaceID,
			data.ID,
		)
