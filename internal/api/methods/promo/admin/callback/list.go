...
var (
	listKey         = "promo.callback.list"
	listDescription = `
Lists promo callback delivery events and their states. Requires the
'promo.callback.list' permission in the target workspace.`
)

// List exposes the promo callback event listing method.
var List = adapter.Method[ListRequest, ListResponse]{
	...
	Handler: func(ctx *adapter.Context, data ListRequest) (ListResponse, error) {
		values, err := services.Promo.Admin.ListCallbackEvents(
			ctx.Context,
			promoadmin.CallbackEventListParams{
				WorkspaceID: data.WorkspaceID,
				Status:      data.Status,
				Page: promoadmin.Page{
					Limit:  data.Limit,
					Offset: data.Offset,
				},
			},
