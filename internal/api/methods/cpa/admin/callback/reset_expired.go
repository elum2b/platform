...
var (
	resetExpiredKey         = "cpa.callback.reset_expired"
	resetExpiredDescription = `
Resets expired CPA callback processing locks for retry. Requires the
'cpa.callback.reset_expired' permission in the target workspace.`
)

// ResetExpired exposes the expired CPA callback processing reset method.
var ResetExpired = adapter.Method[ResetExpiredRequest, ResetExpiredResponse]{
	...
	Handler: func(ctx *adapter.Context, data ResetExpiredRequest) (ResetExpiredResponse, error) {
		affected, err := services.CPA.Admin.ResetExpiredCallbackProcessing(
			ctx.Context,
			data.WorkspaceID,
		)
