...
var (
	listKey         = "cpa.callback.list"
	listDescription = `
Lists CPA callback delivery events and their states. Requires the
'cpa.callback.list' permission in the target workspace.`
)

// List exposes the CPA callback event listing method.
var List = adapter.Method[ListRequest, ListResponse]{
	...
	Handler: func(ctx *adapter.Context, data ListRequest) (ListResponse, error) {
		values, err := services.CPA.Admin.ListCallbackEvents(
			ctx.Context,
			cpaadmin.CallbackEventListParams{
