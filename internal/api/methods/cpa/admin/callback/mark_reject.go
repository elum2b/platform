...
var (
	markRejectKey         = "cpa.callback.mark_reject"
	markRejectDescription = `
Rejects a CPA callback event with a reason. Requires the
'cpa.callback.mark_reject' permission in the target workspace.`
)

// MarkReject exposes the CPA callback rejection method.
var MarkReject = adapter.Method[MarkRejectRequest, MarkRejectResponse]{
	...
	Handler: func(ctx *adapter.Context, data MarkRejectRequest) (MarkRejectResponse, error) {
		affected, err := services.CPA.Admin.MarkCallbackEventReject(
			ctx.Context,
			data.WorkspaceID,
			data.ID,
			data.Reason,
