...
var (
	getKey         = "cpa.callback.get"
	getDescription = `
Returns a CPA callback delivery event. Requires the 'cpa.callback.get'
permission in the target workspace.`
)

// Get exposes the CPA callback event retrieval method.
var Get = adapter.Method[GetRequest, GetResponse]{
	...
	Handler: func(ctx *adapter.Context, data GetRequest) (GetResponse, error) {
		v, err := services.CPA.Admin.GetCallbackEvent(
			ctx.Context,
			data.WorkspaceID,
			data.ID,
		)
