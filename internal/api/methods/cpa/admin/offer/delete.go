...
var (
	deleteKey         = "cpa.offer.delete"
	deleteDescription = `
Deletes a CPA offer and its dependent data. Requires the 'cpa.offer.delete'
permission in the target workspace.`
)

// Delete exposes the CPA offer deletion method.
var Delete = adapter.Method[DeleteRequest, DeleteResponse]{
	...
	Handler: func(ctx *adapter.Context, data DeleteRequest) (DeleteResponse, error) {
		affected, err := services.CPA.Admin.DeleteOffer(
			ctx.Context,
			data.WorkspaceID,
			data.CPAID,
		)
