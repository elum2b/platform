...
var (
	...
	TasksPostgresPassword = env.GetEnvString(
	...
	TasksPostgresDatabase = env.GetEnvString(
	...
	TasksCacheTTLCheck = env.GetEnvDuration(
		"TASKS_CACHE_TTL_CHECK",
	...
	TasksPartnerStartLeaseDuration = env.GetEnvDuration(
		"TASKS_PARTNER_START_LEASE_DURATION",
		0,
	)
	...
	TasksRuntimeScriptCacheTTL = env.GetEnvDuration(
		"TASKS_RUNTIME_SCRIPT_CACHE_TTL",
		0,
	)
	...
	TasksRuntimeMaxHTTPRequests = env.GetEnvInt(
		"TASKS_RUNTIME_MAX_HTTP_REQUESTS",
		0,
	)
	...
	TasksRuntimeJSONBoundary = env.GetEnvBool(
		"TASKS_RUNTIME_JSON_BOUNDARY",
		false,
	)

	// TasksRuntimeStatePoolSize contains Lua state pool size.
	// Env: TASKS_RUNTIME_STATE_POOL_SIZE.
	TasksRuntimeStatePoolSize = env.GetEnvInt(
		"TASKS_RUNTIME_STATE_POOL_SIZE",
		0,
	)
)
